...
typedef struct _jl_task_t {
    JL_DATA_TYPE
    struct _jl_value_t *next; // invasive linked list for scheduler
    struct _jl_value_t *queue; // invasive linked list for scheduler
    struct _jl_value_t *tls;
    struct _jl_value_t *donenotify;
    struct _jl_value_t *result;
    struct _jl_value_t *scope;
    jl_function_t *start;
...
// hidden state:
    ...
    struct _jl_tls_states_t *ptls; // == jl_all_tls_states[tid]
#ifdef USE_TRACY
    const char *name;
#endif
    // saved exception stack
    struct _jl_excstack_t *excstack;
