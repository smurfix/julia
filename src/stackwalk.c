...
static void jl_print_debugloc(jl_debuginfo_t *debuginfo, jl_value_t *func, size_t ip, int inlined) JL_NOTSAFEPOINT
{
    ...
    if (edges_idx) {
        ...
        intptr_t ip2 = stmt.pc;
        if (ip2 != 0)
            jl_print_debugloc(edge, NULL, ip2 - 1, 1);
    }
    intptr_t ip2 = stmt.line;
    if (ip2 == -1)
        ip2 = 0; // broken debug info?
    if (ip2 > 0 && (jl_value_t*)debuginfo->linetable != jl_nothing) {
        // NB: debuginfoloc is 1-based!
        jl_print_debugloc(debuginfo->linetable, func, ip2 - 1, 0);
    }
    else {
...
void jl_print_bt_entry_codeloc(jl_bt_element_t *bt_entry) JL_NOTSAFEPOINT
{
    if (jl_bt_is_native(bt_entry)) {
        ...
        if (jl_is_code_info(code)) {
            ...
            jl_print_debugloc(src->debuginfo, def, ip, 0);
        }
        else {
